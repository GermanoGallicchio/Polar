...
%       po_cfg.sampling.distribution.theta.kappa - [numeric] concentration parameter(s) for theta (kappa >= 0)
%       po_cfg.sampling.distribution.r.mu        - [numeric] mean direction(s) for r density
%       po_cfg.sampling.distribution.r.kappa     - [numeric] concentration parameter(s) for r density (kappa >= 0)
%     For wrappedCauchy distributions:
%       po_cfg.sampling.distribution.theta.mu    - [numeric] mean direction(s) for theta
%       po_cfg.sampling.distribution.theta.rho   - [numeric] concentration parameter(s) for theta (0 <= rho < 1)
%       po_cfg.sampling.distribution.r.mu        - [numeric] mean direction(s) for r density
%       po_cfg.sampling.distribution.r.rho       - [numeric] concentration parameter(s) for r density (0 <= rho < 1)
%     po_cfg.sampling.r_noise             - [numeric] (optional, default=0) multiplicative Gaussian noise level for r (std of noise)
...
fieldLbl = fieldnames(po_cfg);
if ~any(strcmp(fieldLbl,'sampling'))
...
if ~any(strcmp(fieldLbl,'nSamples'))
    error('po_cfg.sampling structure needs to have a "nSamples" field')
end
